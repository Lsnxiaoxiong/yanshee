...
- 先修要求：能够使用基本电脑操作与 VSCode，能安装 Python 与运行简单脚本；无深度学习基础也可，但对基础数学（线性代数/概率）有初步理解更好。
- 目标学员：高校/培训机构学员、机器人爱好者、科研/竞赛团队成员。

### 硬件与软件清单（最小可运行配置）

- 推荐硬件：Yanshee 型号机器人（或兼容机器人）1 台/组；笔记本或台式机（Windows/Mac）；网络连接；USB 摄像头或板载摄像头；麦克风与扬声器。
- 算力建议：训练小规模 YOLO 可用带 CUDA 的 NVIDIA GPU（>=4GB VRAM）；没有 GPU 时可使用小样本训练或云训练（注意费用）。
- 软件与工具：Python 3.9+、pip、VSCode、OpenCV、PyTorch（或 ONNX runtimes）、LabelImg、SSH 客户端（WinSCP/FileZilla/FinalShell）、所需机器人 SDK（YanAPI）。

### 评估与交付（示例）

- 作业/小测：每模块布置小练习（代码 + 运行截图），自动化测试或手动评分。
- 期末项目（分组）：功能实现、代码仓库、部署说明、演示视频与答辩（评分按功能/代码/创新分配）。
- 通过标准示例：能将简单模型部署到机器人并演示感知→决策→动作闭环。
